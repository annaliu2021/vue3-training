<template>
    <div class="container">
        ...
        <div>
            <router-link class="btn btn-outline-secondary my-3" role="button" :to="{ name: 'products' }">
                返回產品列表
            </router-link>
        </div>
        <div class="row gap-5">
            <div class="col col-md-5">
                <div class="product-img border" :style="{ backgroundImage: 'url(' + product.imageUrl + ')' }">
                </div>
                <div class="d-flex mt-4">
                    <div class="product-small-img border me-4"
            ...
            <div class="col">
                ...
                <div>
                    ...
                    <div class="mt-4 d-flex">
                        <select class="form-select me-3" aria-label="Default select example" v-model="number"
                            style="width: 100px;">
                            <option v-for="i in 20" :value="i">{{ i }}</option>
                        </select>
                        <button class="bottom-0 btn btn-danger" type="button" @click="addToCart(product.id)"> 加入購物車
                            <div class=" loading bounceball d-none fade"><span></span><span></span><span></span>
                            </div>
                        </button>
                    </div>
                </div>
...
export default {
    data() {
        ...
        addToCart(productId) {
            ...
            this.$http
                .post(`${VITE_API_ROOT}/api/${VITE_API_PATH}/cart`, { data: { product_id: productId, qty: this.number } })
                .then((response) => {
