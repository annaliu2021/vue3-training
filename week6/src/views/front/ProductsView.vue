    ...
    <div class="container my-3">
        ...
        <ul class="row row-cols-lg-3 list-unstyled p-0">
            <li v-for="item in products" :ked="item.id" class="col d-flex flex-column mb-3">
                <router-link class="products-img border" role="button" :to="`/Product/${item.id}`"
                    :style="{ backgroundImage: 'url(' + item.imageUrl + ')' }">
                    <small>商品內容</small>
                </router-link>
                <div class="p-3 text-center">
                    <p class="m-0">{{ item.title }}</p>
                    <p class="mt-1"><small class="">$ {{ item.price }}元/條</small></p>
                </div>
...
export default {
    data() {
        ...
        addToCart(productId, qty = 1) {
            ...
            this.$http
                .post(`${VITE_API_ROOT}/api/${VITE_API_PATH}/cart`, { data: { product_id: productId, qty: qty } })
                .then((response) => {
